...
# Enfficientnet-b7,ORSSD的权重为59,EORSSD的权重为52, ORSI-4199的权重为45
...
class SEINet(nn.Module):
    def __init__(self, channel=64):
        ...
        self.reduce_sal1 = Reduction(48, channel)
        self.reduce_sal2 = Reduction(80, channel)
        self.reduce_sal3 = Reduction(224, channel)
        self.reduce_sal4 = Reduction(640, channel)

        self.reduce_edg1 = Reduction(48, channel)
        self.reduce_edg2 = Reduction(80, channel)
        self.reduce_edg3 = Reduction(224, channel)
        self.reduce_edg4 = Reduction(640, channel)
    ...
    def forward(self, x):
        ...
        features = self.model.get_blocks(x)

        x_sal1 = self.reduce_sal1(features[0])
        x_sal2 = self.reduce_sal2(features[1])
        x_sal3 = self.reduce_sal3(features[2])
        x_sal4 = self.reduce_sal4(features[3])

        x_edg1 = self.reduce_edg1(features[0])
        x_edg2 = self.reduce_edg2(features[1])
        x_edg3 = self.reduce_edg3(features[2])
        x_edg4 = self.reduce_edg4(features[3])
