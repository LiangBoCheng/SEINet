...
# from model.SEINet_Res2Net50 import SEINet
# from model.SEINet_ResNet50 import SEINet
# from model.SEINet_VGG16 import SEINet
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
           ...
           s1, s1_sig, edg1, s2, s2_sig, edg2, s3, s3_sig, edg3, s4, s4_sig, edg4 = model(images)
           # bce+iou+fmloss
    ...
    save_path = 'models/SEINet_EfficientNetB7/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    if (epoch+1) >= 42:
        torch.save(model.state_dict(), save_path + 'SEINet_EfficientNetB7_ORSSD.pth' + '.%d' % epoch)
